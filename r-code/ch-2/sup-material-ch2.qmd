...
This markdown contains the code to reproduce the main analysis in chapter 2. In it I first show my developement on @kaminskiy2005 method for constructing a joint prior for the two Weibull parameters. I then simulate a dataset that emulates the observation process of the idler-frames and demonstrate the method for imputing the partialy observed lifetimes and missing truncation times acording the the method described in Sec. 2.2 of the Thesis. Lastly, I summarise the results of experiments that explore the method under different values of the simulation parameters, these experiments were run of a virtual machine do to the amount of memory and compute needed.
...
I draw $2000$ sample from the posteriors of each model, using four chains with a burn in of $500$ samples and no thinning. @fig-join-posts shows the posteiror draws of the parameters from the three models as well as the coresponding uncertainty in the Weibull CDF.
...
```{r}
#| label: save-fig-sim-result-factor-plots-means-0.8
#| echo: false
#| output: false
pdf(
  file.path(fig_path, "sim-results-pvalues-0.8.pdf"),
  height = 15,
  width = 10
)
p_sim_result_plot_0.8
...
#| fig-cap: "..."
p_sim_res_elppd <- experiment_results_df_0.8 %>%
  mutate(
    model = lapply(
      experiment_results_df_0.8$model,
      ModelLabeller
...
```{r}
#| label: save-fig-sim-result-elppd-plots-means-0.8
#| echo: false
#| output: false
pdf(
  file.path(fig_path, "sim-results-elppd-0.8.pdf"),
  height = 8.,
  width = 8
)
p_sim_res_elppd_0.8
...
```{r}
#| label: save-fig-sim-result-factor-plots-means-2.0
#| echo: false
#| output: false
pdf(
  file.path(fig_path, "sim-results-pvalues-2.0.pdf"),
  height = 15,
  width = 10
)
p_sim_result_plot_2.0
...
#| fig-cap: "..."
p_sim_res_elppd <- experiment_results_df_2.0 %>%
  mutate(
    model = lapply(
      experiment_results_df_2.0$model,
      ModelLabeller
...
```{r}
#| label: save-fig-sim-result-elppd-plots-means-2.0
#| echo: false
#| output: false
pdf(
  file.path(fig_path, "sim-results-elppd-2.0.pdf"),
  height = 8.,
  width = 8
)
p_sim_res_elppd_2.0

...
```{r}
stan_fit_unknown_lt_rc_large %>%
  mcmc_hist(
    regex_pars = "t_lt_rc_st"
  )
```
...
stan_data_unkown_lt_large <- list(
  ...
  N_lt_rc = nrow(sim_large_df_lt_rc),
  y_obs = sim_large_df_obs$failure_time - sim_large_df_obs$install_time,
  y_rc = sim_large_df_rc$failure_time_obs - sim_large_df_rc$install_time_obs,
  y_lt = sim_large_df_lt$failure_time_obs - sim_large_df_lt$install_time_obs,
...
```{r}
#| label: fit-full-model
stan_fit_lt_r_imputed_inf_large <- sampling(
  stan_model_lt_r_imputed_inf,
  c(
    stan_data_full_large,
    t_1 = qweibull(0.8, beta, eta),
...
```{r}
#| label: fit-no-lt-model
stan_fit_no_lt_inf_large <- sampling(
  stan_model_no_lt_inf,
  c(
    stan_data_no_lt_large,
    t_1 = qweibull(0.8, beta, eta),
...
```{r}
#| label: fit-imp-lt-model
stan_fit_unknown_lt_rc_inf_large <- sampling(
  stan_model_unknown_lt_rc_inf,
  c(
    stan_data_unkown_lt_large,
    t_1 = qweibull(0.8, beta, eta),
...
```{r}
#| label: fig-join-post-and-cdf
p_full_inf_large <- stan_fit_lt_r_imputed_inf_large %>%
  mcmc_scatter(pars = c("beta", "eta")) +
  geom_point(x = beta, y = eta, colour = "red") +
  xlim(0, 5) +
  ylim(0, 5)
p_no_lt_inf_large <- stan_fit_no_lt_inf_large  %>%
  mcmc_scatter(pars = c("beta", "eta")) +
  geom_point(x = beta, y = eta, colour = "red") +
  xlim(0, 5) +
  ylim(0, 5)
p_imp_lt_inf_large <- stan_fit_unknown_lt_rc_inf_large %>%
  mcmc_scatter(pars = c("beta", "eta")) +
  geom_point(x = beta, y = eta, colour = "red") +
  xlim(0, 5) +
  ylim(0, 5)

p_cdf_full_inf_large <- PlotPostCDF(stan_fit_lt_r_imputed_inf_large)
p_cdf_no_lt_inf_large <- PlotPostCDF(stan_fit_no_lt_inf_large)
p_cdf_imp_lt_inf_large <- PlotPostCDF(stan_fit_unknown_lt_rc_inf_large)

plot_grid(
  p_full_inf_large, p_no_lt_inf_large, p_imp_lt_inf_large,
  p_cdf_full_inf_large, p_cdf_no_lt_inf_large, p_cdf_imp_lt_inf_large,
  nrow = 2,
  ncol = 3
)
```

# Strong likelihood and `weak' truncation
...
sim_large_df %>%
  filter(int_censored & right_censored) %>%
  mutate(
    trunc_upper_b = ifelse(
      t_start < (lifetime - (t_end - t_start)),
      t_start,
      (lifetime - (t_end - t_start))
    ),
    trunc_time = t_start - install_time,
    trunc_times_std = trunc_time / trunc_upper_b,
    comp = runif(n())
  ) %>%
  ggplot() +
  geom_histogram(
    aes(comp), fill = alpha("blue", 0.2), colour = alpha("blue", 0.2)
  ) +
  geom_histogram(
    aes(trunc_times_std), fill = alpha("red", 0.2), colour = alpha("red", 0.2)
  )
...
```{r}
SimLtImpData <- function(t_start) {
  sim_large_df_2 <- SimData(
    beta = beta,
    eta = eta,
    n_units = 500,
    t_start = t_start,
  ...
  sim_large_df_obs_2 <- sim_large_df_2 %>%
  filter(!(right_censored | int_censored))
  sim_large_df_rc_2 <- sim_large_df_2 %>%
    filter(right_censored & !int_censored)
  sim_large_df_lt_2 <- sim_large_df_2 %>%
    filter(!right_censored & int_censored)
  sim_large_df_lt_rc_2 <- sim_large_df_2 %>%
    filter(right_censored & int_censored)

  stan_data_unkown_lt_large_2 <- list(
    N_obs = nrow(sim_large_df_obs_2),
    N_rc = nrow(sim_large_df_rc_2),
    N_lt = nrow(sim_large_df_lt_2),
    N_lt_rc = nrow(sim_large_df_lt_rc_2),
    y_obs = as.array(sim_large_df_obs_2$failure_time - sim_large_df_obs_2$install_time),
    y_rc = as.array(sim_large_df_rc_2$failure_time_obs - sim_large_df_rc_2$install_time_obs),
    y_lt = as.array(sim_large_df_lt_2$failure_time_obs - sim_large_df_lt_2$install_time_obs),
    y_lt_rc = as.array(sim_large_df_lt_rc_2$failure_time_obs - sim_large_df_lt_rc_2$install_time_obs),
    t_start = t_start + 10
  )
  return(stan_data_unkown_lt_large_2)
}
test_data_sets <- lapply(
  c(0.5, 1, 2, 4, 8, 16),
  function(t_s) SimLtImpData(t_s)
)
test_stan_fits <- lapply(
  test_data_sets,
  function(test_data) {
...
stan_data_full_large_2 <- list(
  N_obs = nrow(sim_large_df_obs_2),
  N_rc = nrow(sim_large_df_rc_2),
  N_lt = nrow(sim_large_df_lt_2),
  N_lt_rc = nrow(sim_large_df_lt_rc_2),
  y_obs = as.array(sim_large_df_obs_2$failure_time - sim_large_df_obs_2$install_time),
  y_rc = as.array(sim_large_df_rc_2$failure_time_obs - sim_large_df_rc_2$install_time),
  y_lt = as.array(sim_large_df_lt_2$failure_time - sim_large_df_lt_2$install_time),
  t_lt = as.array(sim_large_df_lt_2$install_time_obs - sim_large_df_lt_2$install_time),
  y_lt_rc = as.array(sim_large_df_lt_rc_2$failure_time_obs - sim_large_df_lt_rc_2$install_time),
  t_lt_rc = as.array(sim_large_df_lt_rc_2$install_time_obs - sim_large_df_lt_rc_2$install_time)
)
...
```{r}
#| label: fit-full-model
stan_fit_lt_r_imputed_inf_large_2 <- sampling(
  stan_model_lt_r_imputed_inf,
  c(
    stan_data_full_large_2,
    t_1 = qweibull(0.8, beta, eta),
    t_2 = qweibull(0.98, beta, eta),
    t1_mean = 0.8,
    t1_var = 0.1,
...
```{r}
#| label: fit-imp-lt-model
stan_fit_unknown_lt_rc_inf_large_2 <- sampling(
  stan_model_unknown_lt_rc_inf,
  c(
    stan_data_unkown_lt_large_2,
    t_1 = qweibull(0.8, beta, eta),
    t_2 = qweibull(0.98, beta, eta),
    t1_mean = 0.8,
    t1_var = 0.1,
...
```{r}
p_full_inf_large_2 <- stan_fit_lt_r_imputed_inf_large_2 %>%
  mcmc_scatter(pars = c("beta", "eta")) +
  geom_point(x = beta, y = eta, colour = "red") +
  xlim(0, 5) +
  ylim(0, 5)
p_imp_lt_inf_large_2 <- stan_fit_unknown_lt_rc_inf_large_2 %>%
  mcmc_scatter(pars = c("beta", "eta")) +
  geom_point(x = beta, y = eta, colour = "red") +
  xlim(0, 5) +
  ylim(0, 5)

p_cdf_full_inf_large_2 <- PlotPostCDF(stan_fit_lt_r_imputed_inf_large_2)
p_cdf_imp_lt_inf_large_2 <- PlotPostCDF(stan_fit_unknown_lt_rc_inf_large_2)

plot_grid(
  p_full_inf_large_2, p_imp_lt_inf_large_2,
  p_cdf_full_inf_large_2, p_cdf_imp_lt_inf_large_2,
  nrow = 2,
  ncol = 2
)
```

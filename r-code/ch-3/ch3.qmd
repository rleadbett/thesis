...
```{r}
N_shuts <- 100
N_frames <- 143        # number of units
N_runs <- 10000         # number of sim runs
beta <- 1.1            # Weibull shape
eta <- 1300            # Weibull scale
shut_freq <- (6 * 7)   # frequency of shuts
P_bf <- 0.1            # probability of idler damaging the belt

runs <- lapply(
  1:N_runs,
  function(run) {
    unit_ages <- rep(0, N)
    N_failures <- rep(NA, N_shuts)
    N_belt_failurs <- rep(NA, N_shuts)
    for (shut_int in 1:N_shuts) {
      unit_ages <- unit_ages + shut_freq
      failures <- rbinom(
        n = 10,
        size = 1,
        prob = pweibull(unit_ages, beta, eta) -
          pweibull((unit_ages - shut_freq), beta, eta)
      )
      N_failures[shut_int] <- sum(failures)
      N_belt_failurs[shut_int] <- rbinom(
        n = 1,
        size = sum(failures),
        prob = P_bf
      )
      if(sum(failures) > 0) {
        unit_ages[as.logical(failures)] <- 0
      } 
    }
    sim_df <- data.frame(
      run = run,
      shut = 1:N_shuts,
      idler_failrues = N_failures,
      belt_failures = N_belt_failurs
    )
    return(sim_df)
  }
)
...
runs %>%
  bind_rows() %>%
  group_by(run) %>%
  mutate(Cumsum_belt_failures = cumsum(belt_failures)) %>%
  ungroup() %>%
  group_by(shut) %>%
  summarise(mean_bf = mean(Cumsum_belt_failures)) %>%
  ggplot(aes(x = shut, y = mean_bf)) +
  geom_line()

runs %>%
  bind_rows() %>%
  group_by(shut) %>%
  summarise(mean_if = mean(idler_failrues)) %>%
  ggplot(aes(x = shut, y = mean_if)) +
  geom_line()
```
...
beta <- 1.1
eta <- 1300
# Number of units and their ages
N_units <- 143
unit_ages_start <- rep(0, N_units)
# Time between shutdowns
delta <- (6 * 7)
# Specify probability of unit damaging the belt
P_bf <- 0.05
# For the next 10 shudowns
N_shuts <- 100
K_uf <- rep(NA, N_shuts)
K_bf <- rep(NA, N_shuts)
...
runs %>%
  bind_rows() %>%
  group_by(shut) %>%
  summarise(mean_bf = mean(belt_failures)) %>%
  ggplot(aes(x = shut, y = mean_bf)) +
  geom_line()
...
data.frame(
  shut = rep(1:N_shuts, N_runs),
  run = rep(1:N_runs, each = N_shuts),
  unit_failures = K_uf %>%
    as.numeric(),
  belt_failures = K_bf %>%
    as.numeric()
  ...
  ggplot() +
  stat_lineribbon(aes(x = shut, y = unit_failures)) +
  scale_fill_brewer()
...
data.frame(
  ...
  group_by(run) %>%
  mutate(
    cumulative_BFs = cumsum(belt_failures)
  ) %>%
  ggplot() +
  stat_lineribbon(aes(x = shut, y = cumulative_BFs)) +
  scale_fill_brewer()
```

Fun this for each draw from the posterior.

```{r}
CumulativeBeltFailures <- function(
  beta, eta,        # Weibull parameters
  N_units = 143,    # number of frames
  N_shuts = 50,     # the number of shutdowns to run sim for
  delta = (6 * 7),  # operation time between shuts
  N_runs = 1000,    # number of simulation runs
  P_bf = 0.05       # the probability of a frame failure damaging the belt
  ) {
  unit_ages_start <- rep(0, N_units * N_runs)
  K_uf <- array(rep(NA, N_shuts * N_runs), dim = c(N_shuts, N_runs))
  K_bf <- array(rep(NA, N_shuts * N_runs), dim = c(N_shuts, N_runs))
  ...
  K_bf %>%
    apply(FUN = cumsum, MARGIN = 2) %>%
    apply(FUN = mean, MARGIN = 1) %>%
    return()
}
...
subset_of_draws <- sample(1:nrow(joint_draws), 500)
...
E_N_failures_df %>%
  ggplot() +
  stat_lineribbon(aes(x = shut, y = value)) +
  scale_fill_brewer()
```
...
E_N_failures_df %>%
  ...
  mutate(
    t_p = shut * (6 * 7),
    C = CostFunction(t_p = t_p, F_r = value),
    shut_factor = factor(shut, levels = 1:44, ordered = TRUE)
  ) %>%
  ggplot() +
  stat_halfeye(
    aes(y = shut_factor, x = C),
    .width = c(0.5, 0.9)
  ) +
  theme_minimal()
```

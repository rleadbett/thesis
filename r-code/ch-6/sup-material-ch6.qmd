...
![Example of conveyor belt condition monitoring](belt-cm-example.PNG){#fig-scematic}
...
pdf(
  ...
  p_belt_A_wear_prof +
    geom_hline(yintercept = 25, 
              linetype = "dashed") +
    annotate(geom="text", x=13, y=25,
             label="Soft failure threshold",
             vjust=-1) +
    ylim(-2, 30)
...
where $\Delta y^*_{i, m} = y^*_{i, m} - y^*_{i - 1, m}$ and $\Delta t_{i} = t_{i} - t_{i - 1}$ and **Linear path** $$
\begin{align*}
z_{i, n}|\underline{y}_{i}, \sigma \sim & N(f_i(n), \sigma)               && \text{Data model: FDA} \\ 
f_i(n) =                                & \sum^{M}_{i = 1}b_m(n)y_{i, m}
\end{align*}
...
To fit the model we use the probabalistic programing language Stan [@Stan2022]. Bellow I define each of the
...
Below the BHM in @eq-data-model-gp, @eq-param-model-gp, and @eq-process-model-gp is specified in Stan.
...
As a first pass check, we can look at the summaries of the main parameters and some simple sampling diagnostic values such as effective sample size and $\hat{R}$ [@BDA2020, pp. 285-287].
...
@fig-filtered-spline-coef-gp and @fig-filtered-spline-coef-lm below show a gif cycling through one hundred draws from the posterior distribution of the $y^*_{i, m}$ from each model. Essentialy it is showing us the different posible filterd coeficient pathways. The gray siloets in the background are the other 99 draws that we are cycling through.
...
#|  coeficient paths from the gamma process model. The colours corespond to the basis functions
...
![An animation of 100 different samples of the filtered spline coeficient paths from the gamma process model. The colours corespond to the basis functions in @fig-B-spline-demo.](animation_gp.gif){#fig-filtered-spline-coef-gp}
...
#| fig-cap: 
#|  An animation of 100 different samples of the filtered spline 
#|  coeficient paths from the general path model. The colours corespond to the basis functions
#|  in @fig-B-spline-demo.
#| echo: false
#| eval: false
...
![An animation of 100 different samples of the filtered spline coeficient paths from the general path model. The colours corespond to the basis functions in @fig-B-spline-demo.](animation_lm.gif){#fig-filtered-spline-coef-lm}
:::

@fig-filtered-spline-coef-gp does a good job at demonstrainting that while the posterior mean of each filtered path from the gamma process (like the one included in the paper) may look smooth, there are some fairly jumpy processes in the posterior. Whereas, for the linear general path model in @fig-filtered-spline-coef-lm, all of the draws are perfectly strait (as one would expect).
...
Alternatively, we can calculate the set of smooth spline function for each draw of the $\{y^*_{i, m}\}^{I, M}_{i = 1, m = 1}$ by applying the coeficients to the B-spline basis functions and then calculateing the value of the smooth wear profile across the width of the belt. These distributions are shown in @fig-average-wear-profile-gp and @fig-average-wear-profile-lm for the gamma process and general path models, respectively. What we see is a gradual monotonic growth of the profile with a reasonable amount of uncertainty around what is the average profile along the length of the belt at each observation time.
...
#| fig-cap: The mean wear profile of the belt at each observation time from the fitted gamma process model.
#| echo: false
...
#| fig-cap: The mean wear profile of the belt at each observation time from the fitted general path model.
#| echo: false
